...
# Hugging Face client. The newer "Inference Providers" system routes each
# model to whichever backend(s) actually host it (hf-inference, together,
# featherless-ai, etc). provider="auto" lets the client pick automatically -
# forcing a fixed provider like "hf-inference" fails for models that aren't
# hosted there (many chat models, including several Qwen variants, are NOT
# on hf-inference - only on together/featherless-ai).
